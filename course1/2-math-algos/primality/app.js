...
// we have also seen O(sqrt(n)), but if we consider it as O(n) that would be ok. just to be a bit more specific, it is worth it to take a look at what's going on in the loop (check how the iterator changes with each iteration, and what your exit/runtime condition is; if you are incrementing the iterator by more than one, or if the exit condition doens't run through all the cases, then probably you are decreasing the number of iterations, so you are doing a bit better than O(n))
